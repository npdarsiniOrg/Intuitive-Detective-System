...
/**
  * Created by Harsha on 19-11-2016.
  */
object ImageClassification_DT {
  def main(args: Array[String]) {
    val topic = "featuresToSpark"
//    val kConsumer = new KafkaConsumer("localhost:2181", "testgroup", topic)
//    kConsumer.testConsumer()
    val IMAGE_CATEGORIES = Array("priya", "harsha", "aish")
//    System.setProperty("hadoop.home.dir", "E:\\winutils")
    ...
    val parsedData = train.map { line =>
      ...
      val category = parts(0).toString match {
        case "priya" => 0
        case "harsha" => 1
        case "aish" => 2
        case _ => 3
      }
      LabeledPoint(category, Vectors.dense(parts(1).split(' ').map(_.toDouble)))
    }
...
//    val uploadToMongo = new MongoUpload
